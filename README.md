...
* **Целевая позиция** + **Отправить цель** — публикует команду
  `target_position` в `digital_twin/actuator/command`. PID отрабатывает
  новую уставку.
* **Пауза / Продолжить** — замораживает интегратор; кадры в MQTT не
  обновляются, но публикация не останавливается.
* **Сброс** — обнуляет все модели (x=0, T=25 °C, история графиков
  очищается). Состояние паузы сохраняется.
* **Запись** — начинает/останавливает запись телеметрии в файл (см. ниже).
* **АВАРИЙНЫЙ СТОП** — обнуляет силу мотора. Чтобы продолжить, отправьте
  новую цель (она автоматически снимет стоп).
* В 3D-сцене работает мышь: левая кнопка — вращение, правая — панорама,
  колесо — зум.
* Таймер в верхнем правом углу считает время с момента первого кадра
  (или последнего сброса). Останавливается на паузе.

## Запись телеметрии

При нажатии **Запись** симулятор создаёт каталог
`recordings/<YYYY-MM-DD_HH-MM-SS>/` и пишет туда:

* `telemetry.jsonl` — по одному `TelemetryFrame` в JSON на строку, ~200
  кадров в секунду (частота интегратора).
* `meta.json` — снимок всех параметров моделей в момент старта записи —
  чтобы запись была воспроизводима.

Каталог `recordings/` исключён из git.

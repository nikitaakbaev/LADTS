...
| `emergency_stop`  | bool   | —        | `true` — вязкий тормоз, привод стоит замком   |
| `paused`          | bool   | —        | `true` — приостановить интегратор; публикация продолжается с замороженным кадром |
| `reset`           | bool   | —        | `true` — пересоздать все модели (x=0, T=T_env, история обнуляется); состояние паузы сохраняется |
| `record`          | bool   | —        | `true` — начать запись телеметрии в `recordings/<timestamp>/`, `false` — остановить |
